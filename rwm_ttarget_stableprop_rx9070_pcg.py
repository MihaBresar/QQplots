# rwm_ttarget_stableprop_rx9070_pcg.py
# RWM targeting Student-t (nu_target=5, finite first three moments).
# Proposals: symmetric alpha-stable (alpha=0.1) via CMS.
...
PROPOSAL_ALPHA    =        0.10     # 0.1 is ultra heavy-tailed; tune for acceptance
PROPOSAL_SCALE    =        1.0      # step size multiplier; tune for 10–40% accept
...
// Numerically stable with log/exp and clamping.
...
__kernel void rwm_stable_chunk2_pcg(
    ...
    const int K)
{
    ...
    for (int it=0; it<K; ++it){
        ...
        // ---- step B ----
        step  = prop_scale * sample_symmetric_stable(&st, inc, alpha);
        xprop = xi + step;
...
def run():
    ...
    kernel = cl.Kernel(prg, "rwm_stable_chunk2_pcg")
    ...
    kernel.set_arg(9,  np.float32(PROPOSAL_ALPHA))
    kernel.set_arg(10, np.float32(PROPOSAL_SCALE))
    kernel.set_arg(11, np.float32(NU_TARGET))
    kernel.set_arg(12, np.float32(TAU_TARGET))
    # arg 13 = K, set per launch
    ...
    with open(os.path.join(scriptdir, "ergodic_average_abs_RWM_ttarget_stable_pcg.csv"), "w", newline="") as f:
        w = csv.writer(f); w.writerows([[v] for v in erg_abs])

    with open(os.path.join(scriptdir, "ergodic_average_indicator_RWM_ttarget_stable_pcg.csv"), "w", newline="") as f:
        w = csv.writer(f); w.writerows([[v] for v in erg_ind])

    with open(os.path.join(scriptdir, "acceptance_chain0_RWM_ttarget_stable_pcg.txt"), "w") as f:
        f.write(f"accepts={acc}, proposals={props}, accept_rate={acc_rate:.6f}\n")

    print("\nCSV files written:")
    print("  ergodic_average_abs_RWM_ttarget_stable_pcg.csv")
    print("  ergodic_average_indicator_RWM_ttarget_stable_pcg.csv")
    print("  acceptance_chain0_RWM_ttarget_stable_pcg.txt")
